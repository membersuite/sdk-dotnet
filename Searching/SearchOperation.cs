...
namespace MemberSuite.SDK.Searching
{
    ...
    public abstract class SearchOperation
    {
        ...
        /// </summary>
        /// <returns></returns>
        /// <remarks>We only want to include stuff that's relevant to the search strucutre -
        /// that's why ValuesToOperateOn isn't included!</remarks>
        ...
        /// </summary>
        /// <remarks>It's CRITICAL that this is NOT serialized, otherwise it will cause exceptions at the WCF level
        /// as WCF's serializer tries to ready it for the wire</remarks>
        [NonSerialized]
        private SearchOperationGroup _parent;
        ...
        public virtual void PostSerializationCleanse()
        {
            ...
            for (int i = 0; i < ValuesToOperateOn.Count; i++)
            {
        ...
        public virtual List<string> GetMergeCriteriaFields()
        {
            ...
            foreach (var o in ValuesToOperateOn)
            {
                ...
                if (s.StartsWith("##") && s.EndsWith("##") && !result.Contains(s, StringComparer.InvariantCultureIgnoreCase))
                    result.Add(s);
            }
        ...
        /// </value>
        public virtual bool SupportsParameterization { get { return false; } }

        public virtual int NumberOfParameters { get { return 1; } }
